...
class SACAgent(AbstractAgent):
    ...
    def __init__(self, env, cfg, args, loggers=tuple(), save_episodes=True, save_batch_size=256, 
        ...
        #self.obs_dim = self.cfg['latent_dims'] \
        #        if self.cfg['encoder'] else self.env.observation_space.shape
        ...
        ## vision encoder
        if 'resnet' in self.cfg['use_encoder_type']:
            self.backbone = resnet18(pretrained=self.cfg['resnet']['pretrained'])
        elif self.cfg['use_encoder_type'] == 'vae':
            self.backbone = VAE(
        ...
        elif self.cfg['use_encoder_type'] == 'vae_small':
            self.backbone = VAE(
        ...
        if self.cfg['checkpoint'] and self.cfg['load_checkpoint']:
            #self.actor_critic = torch.load(self.cfg['checkpoint'])
            self.actor_critic.load_state_dict(torch.load(self.cfg['checkpoint']))
            self.episode_num = int(''.join(filter(str.isdigit, os.path.split(self.cfg['checkpoint'])[-1])))
            self.file_logger(f"Loaded checkpoint, {self.cfg['checkpoint']}")
    ...
    def _encode(self, o):
        ...
        if self.cfg['use_encoder_type'] == 'resnet':
            img = self.transform(np.array(img).squeeze(0)).float().to(DEVICE)
            speed = torch.tensor((state[4]**2 + state[3]**2 + state[5]**2)**0.5).float().reshape(1, -1).to(DEVICE)
            img_embed = self.backbone(img.unsqueeze(0))
            out = torch.cat([img_embed, speed], dim = -1).squeeze(0) # BS X 513
            self.using_speed = 1
        elif self.cfg['use_encoder_type'] == 'vae':
            img = torch.as_tensor(cv2.resize(img, (im_w, im_h)), dtype=torch.float32, device=DEVICE)
            img = img / 255
            out, _, _ = self.backbone.encode(img.view(1, 3, im_w, im_h)) # BS X 32
            self.using_speed = 0
        elif self.cfg['use_encoder_type'] == 'vae_small':
    ...
    def eval(self, n_eps):
        ...
        if (ep_ret > self.best_ret and ep_ret > 250):
            self.file_logger(f'New best episode reward of {round(ep_ret, 1)}!')
            ...
            self.file_logger('Periodic save: save_freq of {}'.format(self.cfg['save_freq']))
        ...
        if self.best_pct < info['metrics']['pct_complete']:
            if (self.best_pct<93) & (info['metrics']['pct_complete']>93):
...
#        path_name = f"{self.cfg['save_path']}il_episode_{e}.pt"
#        torch.save(self.model.state_dict(), path_name)
#
#    def load_model(self):
#        path = f"{self.cfg['save_path']}{self.cfg.checkpoint_name}"
    ...
    def sac_train(self):
        ...
        for t in range(self.t_start, self.cfg['total_steps']):
            ...
            if d or (ep_len == self.cfg['max_ep_len']):
                ...
                # as long as the episode resulted in "success"
                if self.cfg['record_experience'] and self.metadata['info']['success']:
                    self.file_logger("Success -- writing experience")
                    self.save_queue.put(experience)
