...
def main(params, seed, args, t_start):
    ...
    if not os.path.exists(save_path):
        ...
        os.makedirs(f"{save_path}/safety", mode=0o777, exist_ok=True)
    ...
    agent = SafeSACAgent(env, sac_kwargs, args, loggers=loggers, save_episodes=sac_kwargs['save_episodes'], 
                        t_start = t_start)
...
if __name__ == "__main__":
   ...
   #     pass
    ...
    if opts.load:
        ...
        params['sac_kwargs']['experiment_name'] = params['sac_kwargs']['checkpoint'].split('/')[-1][:-10]
        print(f"Load checkpoint from {params['sac_kwargs']['experiment_name']}")
        seed = int(params['sac_kwargs']['experiment_name'].split('_')[-3].split('-')[-1])
        t_start = 20*5000
        #t_start = int(params['sac_kwargs']['experiment_name'].split('_')[-1]) * 5000
        print(seed, t_start)
